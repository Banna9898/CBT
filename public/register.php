<?php require_once __DIR__ . '/../config/db.php'; if($_SERVER['REQUEST_METHOD']==='POST'){ $name=trim($_POST['name']); $email=trim($_POST['email']); $phone = trim($_POST['phone']); $password=$_POST['password']; if(!filter_var($email, FILTER_VALIDATE_EMAIL)){ $err='Invalid email'; } else { $stmt=$pdo->prepare('SELECT id FROM users WHERE email=?'); $stmt->execute([$email]); if($stmt->fetch()) $err='Email already registered'; } if(empty($err)){ $hash=password_hash($password, PASSWORD_DEFAULT); $stmt=$pdo->prepare('INSERT INTO users (name,email,phone,password,role,status) VALUES (?,?,?,?,?,?) RETURNING id,email,name,role,status'); $stmt->execute([$name,$email,$phone,$hash,'user','pending']); $user=$stmt->fetch(PDO::FETCH_ASSOC); $msg='Registered successfully. Await admin approval.'; } } require_once __DIR__ . '/../includes/header.php'; ?>
